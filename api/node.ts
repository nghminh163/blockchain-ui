...
import { URL_SERVER, SSR_URL_SERVER } from "../constants";
...
export async function getPeers(page = 1, perPage = 10) {
  const { data } = await axios.get<PeerResponse>(
    `${SSR_URL_SERVER}/node/peers?page=${page}&per_page=${perPage}`
  );
  return data;
}
...
export async function getMempoolSSR() {
  const { data } = await axios.get<BlockTx[]>(`${SSR_URL_SERVER}/node/mempool`);
  return data;
}
